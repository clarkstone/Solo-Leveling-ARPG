#include "P19_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP19_OMEGA_STATE_SYSTEM::UP19_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 100.0f;
    OmegaStateThreshold = 100.0f;
...
void UP19_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP19_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP19_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP19_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP19_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP19_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP19_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP19_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP19_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP19_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP19_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP19_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP19_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP19_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP19_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP19_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 10.0f;
    FinalStateAuthority = OmegaStateLevel * 10.2f;
    StateTransitions = OmegaStateLevel * 9.8f;
    UltimateConsciousness = OmegaStateLevel * 9.9f;
    FinalUltimacy = OmegaStateLevel * 10.1f;
}

void UP19_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
