#include "P05_CORE_GOD_MODE.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP05_CORE_GOD_MODE::UP05_CORE_GOD_MODE()
{
    PrimaryComponentTick.bCanEverTick = false;
    GodModeLevel = 0;
    MaxGodModeLevel = 100;
    bIsGodModeActive = false;
    GodModePowerCost = 20.0f;
    GodModeThreshold = 20.0f;
...
void UP05_CORE_GOD_MODE::BeginPlay()
{
    Super::BeginPlay();
    UpdateGodModeStats();
}

void UP05_CORE_GOD_MODE::ActivateGodMode()
{
...
void UP05_CORE_GOD_MODE::DeactivateGodMode()
{
    if (bIsGodModeActive)
    {
        bIsGodModeActive = false;
...
void UP05_CORE_GOD_MODE::SetGodModeLevel(int32 NewLevel)
{
    if (NewLevel >= 0 && NewLevel <= MaxGodModeLevel)
    {
        ...
        if (GodModeLevel >= MaxGodModeLevel)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("God Mode Max Level Reached!"));
        }
    }
}

void UP05_CORE_GOD_MODE::UseGodModePower()
{
...
void UP05_CORE_GOD_MODE::PerformGodModeAbility()
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("God Mode Level too low to perform ability. Required: %.0f, Current: %d"), GodModeThreshold, GodModeLevel);
    }
    else
    {
        UE_LOG(LogTemp, Warning, TEXT("God Mode is not active, cannot perform ability."));
    }
}

bool UP05_CORE_GOD_MODE::IsGodModeActive() const
{
    return bIsGodModeActive;
}

int32 UP05_CORE_GOD_MODE::GetGodModeLevel() const
{
    return GodModeLevel;
}

float UP05_CORE_GOD_MODE::GetGodModePowerCost() const
{
    return GodModePowerCost;
}

void UP05_CORE_GOD_MODE::BecomeInvincible()
{
    if (bIsGodModeActive)
    {
        float InvincibilityPower = Invincibility * (GodModeLevel / 100.0f);
        UE_LOG(LogTemp, Warning, TEXT("Becoming Invincible with power: %.2f"), InvincibilityPower);
        // Apply invincibility effects
    }
}

void UP05_CORE_GOD_MODE::WieldDivineAuthority()
{
    if (bIsGodModeActive)
    {
        float AuthorityPower = DivineAuthority * (GodModeLevel / 100.0f);
        UE_LOG(LogTemp, Warning, TEXT("Wielding Divine Authority with power: %.2f"), AuthorityPower);
        // Apply divine authority effects
    }
}

void UP05_CORE_GOD_MODE::AccessOmniscience()
{
    if (bIsGodModeActive)
    {
...
void UP05_CORE_GOD_MODE::AchieveOmniscience()
{
    if (bIsGodModeActive)
    {
        float OmnisciencePower = Omniscience * (GodModeLevel / 100.0f);
...
bool UP05_CORE_GOD_MODE::CanActivateGodMode() const
{
    return GodModeLevel > 0;
}

void UP05_CORE_GOD_MODE::UpdateGodModeStats()
{
    // Update god mode properties based on level
    GodModePower = GodModeLevel * 2.5f;
    Invincibility = GodModeLevel * 2.3f;
    DivineAuthority = GodModeLevel * 2.8f;
    Omniscience = GodModeLevel * 2.6f;
}

void UP05_CORE_GOD_MODE::OnGodModeStateChanged()
{
