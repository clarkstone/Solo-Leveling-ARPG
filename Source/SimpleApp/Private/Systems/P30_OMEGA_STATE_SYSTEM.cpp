#include "P30_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP30_OMEGA_STATE_SYSTEM::UP30_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 155.0f;
    OmegaStateThreshold = 155.0f;
...
void UP30_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP30_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP30_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP30_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP30_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP30_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP30_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP30_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP30_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP30_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP30_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP30_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP30_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP30_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP30_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP30_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 15.5f;
    FinalStateAuthority = OmegaStateLevel * 15.7f;
    StateTransitions = OmegaStateLevel * 15.3f;
    UltimateConsciousness = OmegaStateLevel * 15.4f;
    FinalUltimacy = OmegaStateLevel * 15.6f;
}

void UP30_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
