#include "P26_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP26_OMEGA_STATE_SYSTEM::UP26_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 135.0f;
    OmegaStateThreshold = 135.0f;
...
void UP26_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP26_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP26_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP26_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP26_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP26_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP26_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP26_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP26_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP26_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP26_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP26_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP26_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP26_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP26_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP26_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 13.5f;
    FinalStateAuthority = OmegaStateLevel * 13.7f;
    StateTransitions = OmegaStateLevel * 13.3f;
    UltimateConsciousness = OmegaStateLevel * 13.4f;
    FinalUltimacy = OmegaStateLevel * 13.6f;
}

void UP26_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
