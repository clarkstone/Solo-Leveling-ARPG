#include "P20_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP20_OMEGA_STATE_SYSTEM::UP20_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 105.0f;
    OmegaStateThreshold = 105.0f;
...
void UP20_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP20_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP20_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP20_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP20_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP20_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP20_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP20_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP20_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP20_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP20_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP20_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP20_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP20_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP20_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP20_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 10.5f;
    FinalStateAuthority = OmegaStateLevel * 10.7f;
    StateTransitions = OmegaStateLevel * 10.3f;
    UltimateConsciousness = OmegaStateLevel * 10.4f;
    FinalUltimacy = OmegaStateLevel * 10.6f;
}

void UP20_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
