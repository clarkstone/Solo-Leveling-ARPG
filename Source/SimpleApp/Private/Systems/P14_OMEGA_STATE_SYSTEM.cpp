#include "P14_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP14_OMEGA_STATE_SYSTEM::UP14_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 75.0f;
    OmegaStateThreshold = 75.0f;
...
void UP14_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP14_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP14_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP14_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP14_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP14_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP14_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP14_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP14_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP14_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
        UE_LOG(LogTemp, TEXT("Exercising Omega Phase Ultimacy with power: %.2f"), UltimacyPower);
        // Apply omega phase ultimacy effects
    }
}

void UP14_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
        UE_LOG(LogTemp, TEXT("Achieving Omega Phase Final Ultimacy with power: %.2f"), UltimacyPower);
        // Apply omega phase final ultimacy effects
    }
}

void UP14_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
        float TransitionPower = StateTransitions * (OmegaStateLevel / 100.0f);
        UE_LOG(LogTemp, TEXT("Controlling Final State Transitions with power: %.2f"), TransitionPower);
        // Apply final state transition control effects
    }
}

void UP14_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP14_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = FinalUltimacy * (OmegaStateLevel / 100.0f);
        UE_LOG(LogTemp, TEXT("Wielding Final Ultimacy with power: %.2f"), UltimacyPower);
        // Apply final ultimacy effects
    }
}

bool UP14_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP14_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 7.5f;
    FinalStateAuthority = OmegaStateLevel * 7.7f;
    StateTransitions = OmegaStateLevel * 7.3f;
    UltimateConsciousness = OmegaStateLevel * 7.4f;
    FinalUltimacy = OmegaStateLevel * 7.6f;
}

void UP14_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
