#include "P16_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP16_OMEGA_STATE_SYSTEM::UP16_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 85.0f;
    OmegaStateThreshold = 85.0f;
...
void UP16_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP16_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP16_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP16_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
...
void UP16_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP16_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP16_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP16_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP16_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP16_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP16_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP16_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP16_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP16_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP16_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP16_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 8.5f;
    FinalStateAuthority = OmegaStateLevel * 8.7f;
    StateTransitions = OmegaStateLevel * 8.3f;
    UltimateConsciousness = OmegaStateLevel * 8.4f;
    FinalUltimacy = OmegaStateLevel * 8.6f;
}

void UP16_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
