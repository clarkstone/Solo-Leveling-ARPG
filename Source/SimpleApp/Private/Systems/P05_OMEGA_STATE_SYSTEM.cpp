#include "P05_OMEGA_STATE_SYSTEM.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP05_OMEGA_STATE_SYSTEM::UP05_OMEGA_STATE_SYSTEM()
{
    PrimaryComponentTick.bCanEverTick = false;
    OmegaStateLevel = 0;
    MaxOmegaStateLevel = 100;
    bIsOmegaStateActive = false;
    OmegaStatePowerCost = 30.0f;
...
void UP05_OMEGA_STATE_SYSTEM::BeginPlay()
{
    Super::BeginPlay();
    UpdateOmegaStateStats();
}

void UP05_OMEGA_STATE_SYSTEM::ActivateOmegaState()
{
...
void UP05_OMEGA_STATE_SYSTEM::DeactivateOmegaState()
{
...
void UP05_OMEGA_STATE_SYSTEM::SetOmegaStateLevel(int32 NewLevel)
{
    if (NewLevel >= 0 && NewLevel <= MaxOmegaStateLevel)
    {
        ...
        UpdateOmegaStateStats();
        OnOmegaStateChanged.Broadcast(OldLevel, OmegaStateLevel);
...
void UP05_OMEGA_STATE_SYSTEM::UseOmegaStatePower()
{
...
void UP05_OMEGA_STATE_SYSTEM::PerformOmegaStateAbility()
{
...
bool UP05_OMEGA_STATE_SYSTEM::IsOmegaStateActive() const
{
    return bIsOmegaStateActive;
}

int32 UP05_OMEGA_STATE_SYSTEM::GetOmegaStateLevel() const
{
    return OmegaStateLevel;
}

float UP05_OMEGA_STATE_SYSTEM::GetOmegaStatePowerCost() const
{
    return OmegaStatePowerCost;
}

void UP05_OMEGA_STATE_SYSTEM::ExerciseOmegaPhaseUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
void UP05_OMEGA_STATE_SYSTEM::AchieveOmegaPhaseFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
        float UltimacyPower = OmegaPhaseUltimacy * (OmegaStateLevel / 100.0f);
...
void UP05_OMEGA_STATE_SYSTEM::ControlFinalStateTransitions()
{
    if (bIsOmegaStateActive)
    {
...
void UP05_OMEGA_STATE_SYSTEM::AccessUltimateConsciousness()
{
    if (bIsOmegaStateActive)
    {
...
void UP05_OMEGA_STATE_SYSTEM::WieldFinalUltimacy()
{
    if (bIsOmegaStateActive)
    {
...
bool UP05_OMEGA_STATE_SYSTEM::CanActivateOmegaState() const
{
    return OmegaStateLevel > 0;
}

void UP05_OMEGA_STATE_SYSTEM::UpdateOmegaStateStats()
{
    // Update omega state properties based on level
    OmegaPhaseUltimacy = OmegaStateLevel * 4.0f;
    FinalStateAuthority = OmegaStateLevel * 4.2f;
    StateTransitions = OmegaStateLevel * 3.8f;
    UltimateConsciousness = OmegaStateLevel * 3.9f;
    FinalUltimacy = OmegaStateLevel * 4.1f;
}

void UP05_OMEGA_STATE_SYSTEM::OnOmegaStateChanged()
{
