#include "P05_CORE_CHAR_MODELS.h"
#include "GameFramework/Character.h"
#include "Kismet/GameplayStatics.h"

// Sets default values for this component's properties
UP05_CORE_CHAR_MODELS::UP05_CORE_CHAR_MODELS()
{
    PrimaryComponentTick.bCanEverTick = false;
    CharacterModelsLevel = 0;
    MaxCharacterModelsLevel = 100;
    bIsCharacterModelsActive = false;
    CharacterModelsPowerCost = 20.0f;
    CharacterModelsThreshold = 20.0f;
...
void UP05_CORE_CHAR_MODELS::BeginPlay()
{
    Super::BeginPlay();
    UpdateCharacterModelsStats();
}

void UP05_CORE_CHAR_MODELS::ActivateCharacterModels()
{
...
void UP05_CORE_CHAR_MODELS::DeactivateCharacterModels()
{
...
void UP05_CORE_CHAR_MODELS::SetCharacterModelsLevel(int32 NewLevel)
{
...
void UP05_CORE_CHAR_MODELS::UseCharacterModelsPower()
{
...
void UP05_CORE_CHAR_MODELS::PerformCharacterModelsAbility()
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("Character Models is not active, cannot perform ability."));
    }
}

bool UP05_CORE_CHAR_MODELS::IsCharacterModelsActive() const
{
    return bIsCharacterModelsActive;
}

int32 UP05_CORE_CHAR_MODELS::GetCharacterModelsLevel() const
{
    return CharacterModelsLevel;
}

float UP05_CORE_CHAR_MODELS::GetCharacterModelsPowerCost() const
{
    return CharacterModelsPowerCost;
}

void UP05_CORE_CHAR_MODELS::CreateModels()
{
    if (bIsCharacterModelsActive)
    {
        float CreationPower = ModelCreation * (CharacterModelsLevel / 100.0f);
        UE_LOG(LogTemp, Warning, TEXT("Creating Models with power: %.2f"), CreationPower);
        // Apply model creation effects
    }
}

void UP05_CORE_CHAR_MODELS::AccessAnimationSystem()
{
    if (bIsCharacterModelsActive)
    {
...
void UP05_CORE_CHAR_MODELS::ApplyVisualEffects()
{
    if (bIsCharacterModelsActive)
    {
...
bool UP05_CORE_CHAR_MODELS::CanActivateCharacterModels() const
{
    return CharacterModelsLevel > 0;
}

void UP05_CORE_CHAR_MODELS::UpdateCharacterModelsStats()
{
    // Update character models properties based on level
    ModelCreation = CharacterModelsLevel * 2.0f;
    AnimationSystem = CharacterModelsLevel * 1.8f;
    VisualEffects = CharacterModelsLevel * 1.9f;
}

void UP05_CORE_CHAR_MODELS::OnCharacterModelsStateChanged()
{
